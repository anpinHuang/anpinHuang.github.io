...
$$E[I_i|Z] = \mathcal{N}_{\epsilon}(\frac{\mathcal{N}_A^{-1}(PD_i)-\sqrt{\rho}Z}{\sqrt{1-\rho}})$$

Assuming PD, EAD and LGD are independent, then the expected loss (EL) for the portfolio is:

$$E[L_p|Z] = \sum_{i=1}^{n} EAD_i \cdot LGD_i \cdot \mathcal{N}_{\epsilon}(\frac{\mathcal{N}_A^{-1}(PD_i)-\sqrt{\rho}Z}{\sqrt{1-\rho}})$$

Now we can calculate the value-at-risk (VaR) given a specified condifence level $\alpha$:

$$VaR(\alpha) = \sum_{i=1}^{n} EAD_i \cdot LGD_i \cdot \mathcal{N}_{\epsilon}(\frac{\mathcal{N}_A^{-1}(PD_i)-\sqrt{\rho} \mathcal{N}_Z^{-1}(1-\alpha)}{\sqrt{1-\rho}})$$
In practice, 
