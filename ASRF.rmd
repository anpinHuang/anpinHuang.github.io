...
If there are multiple ratings in the pool, then $PD_{TtC}$ of the portfolio is not constant (portfolio changes over time). Therefore, $\rho$ would be to be estimated by maximum likelihood in this case. 
...
* where $r$ is the rating, $d$ is the default state (rating) and $MP^{PiT}_{r \to d}$ is the PiT migration probability from rating $r$ to default $d$, and is therefore also $PD_{PiT}$.
